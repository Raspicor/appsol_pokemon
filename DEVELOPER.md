# PikaPet 개발자 노트 — 리버스 엔지니어링 및 macOS 포팅 분석
...
**체력바.** 게임은 체력/경험치를 `ttk.Progressbar`로 그립니다. aqua의 그 위젯은
네이티브 트랙을 6px 높이로만 그리고, 위젯의 남은 높이(16px 중 10px)를 시스템
색으로 채웁니다. 다크 모드에서 그 색이 거의 검정이라 **얇은 파란 선 위아래로
검은 띠가 5px씩** 남습니다. 실측한 세로 단면:
...
**ttk 스타일로는 못 고칩니다.** `background` / `troughcolor` / `bordercolor` /
`lightcolor` / `darkcolor` 를 어떻게 조합해도 aqua는 전부 무시했습니다 (5가지
조합을 그려서 확인, 전부 동일한 `(28,28,28)`). 테마를 `clam`으로 바꾸면 색이
먹지만, 그러면 게임이 쓰는 `ttk.Combobox` 2개까지 네이티브 드롭다운을 잃습니다.

그래서 `ttk.Progressbar`만 Tk로 직접 그리는 `MacProgressBar`로 바꿔치웁니다.
게임이 쓰는 API는 `length`/`maximum`/`value` 뿐입니다(11곳 전부 확인). 높이는
네이티브와 같은 16px로 두어 레이아웃이 밀리지 않게 합니다. 고친 뒤 바 주변의
검은 픽셀은 2100개 → **0개**, 채움 비율은 30%/75% 지정에 62/210(30%),
156/210(74%)로 나왔습니다.

**모서리 조각.** compact 전투 창 오른쪽 아래의 `⇲`는 크기 조절 핸들입니다
(pet.py:10070, `cursor='sizing'`, `_resize_press` 바인딩). 헤더와 같은
주황(`#e8a53a`)인데 macOS는 창 모서리를 둥글게 깎아서 그 주황 사각형이 잘려
나간 조각처럼 보입니다. Windows는 모서리가 각져서 깔끔하게 맞물립니다.
기능은 그대로 두고 색만 창 배경에 맞춰 글리프만 은은하게 남깁니다.
...
143개 전부 통과합니다. 실제로 두 건의 버그를 잡았습니다:
...
run/                 macOS 실행 환경
  ...
  test_macui.py        글리프 보정 / 색 버튼 / 대비 보정 / 체력바 테스트 57개
