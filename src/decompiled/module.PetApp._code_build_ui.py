# module.PetApp._code_build_ui
# source line 12927
# Recovered from bytecode; default argument values are not shown.

def _code_build_ui(self, gctx):
    win = gctx['win']
    for w in win.winfo_children():
        w.destroy()
    opp_name = gctx.get('opp_name', '상대')
    ui_title = gctx.get('ui_title', f'''🔑 코드 대결 - vs {opp_name}''')
    win.title(ui_title)
    ...
    try:
        ...
        None(body, text = ui_title, font = ('맑은 고딕', 12, 'bold')).pack(pady = (2, 6))
        reward_banner = gctx.get('reward_banner')
        if reward_banner:
            None(body, text = reward_banner, font = ('맑은 고딕', 9, 'bold'), fg = '#1a7a3a', bg = '#eafbea', wraplength = 400, justify = 'left', padx = 8, pady = 6, relief = 'solid', bd = 1).pack(fill = 'x', pady = (0, 8))
        sprite_row = None(body)
        sprite_row.pack(pady = (0, 6))
        ...
        e_card = None(body, text = f'''상대 ({opp_name})''', padx = 8, pady = 6)
        e_card.pack(fill = 'x')
        e_name_var = None()
        ...
        log_var = None(value = f'''코드 대결 시작! vs {opp_name} - 순서대로 한 마리씩 싸워요.''')
        ...
        btn_row.pack()
        hide_ultimate = bool(gctx.get('hide_ultimate'))
        hide_attack = bool(gctx.get('hide_attack'))
        ult_btn = None
        if not hide_ultimate:
            ult_btn = None(btn_row, text = '✨ 필살기', width = 16, wraplength = 100, justify = 'center', command = (lambda : self._gym_action(gctx, 'ultimate')))
        atk_btn = None
        if not hide_attack:
        ...
        (ult_btn, atk_btn, def_btn, give_btn), b
